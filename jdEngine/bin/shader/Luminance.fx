...
Texture2D txLights : register(t3);
...
PS_OUTPUT PS(PS_INPUT input)
{
  ...
  //float4 dif = txDifuse.Sample(samLinear, input.Tex);
  //float4 spec = txSpecular.Sample(samLinear, input.Tex);
  //float4 ambient = txAmbient.Sample(samLinear, input.Tex);
  //float4 lights = dif + spec +ambient;
  //Texture color Sampler
  ...
  float4 addLight = txLights.Sample(samLinear, input.Tex);
  float4 lights = dif + spec + ambient;
 ...
 // Calcule Luminance
  float Luminance = dot(addLight, LuminanceFactor);
